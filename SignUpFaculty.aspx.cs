...
namespace SOMS
{
    public partial class SignUpFaculty : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                role.Items.Insert(0, new ListItem("SELECT ROLE", ""));
        ...
        private void LoadDropdownItems(string auth)
        {

            using (SqlConnection conn = new SqlConnection(connStr))
            {
                string query = "SELECT Organization FROM TB_Organization WHERE Authority = @auth AND Status = 1";
                SqlCommand cmd = new SqlCommand(query, conn);
                cmd.Parameters.AddWithValue("@auth", auth); //use session email
        ...
        {
            if (role.SelectedValue == "ADM")
            {
                LoadDropdownItems("ADM");
            }
            else if (role.SelectedValue == "FAC")
            {
                LoadDropdownItems("FAC");
            }
            else
            {
                organization.Items.Clear(); // Clear the dropdown if no role is selected
                organization.Items.Insert(0, new ListItem("SELECT ROLE FIRST", ""));
                organization.Items[0].Attributes["disabled"] = "disabled";
            }
        }
    }
