    ...
    public partial class Calendar : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                bool isAuthenticated = Session["UserAuthenticated"] != null && (bool)Session["UserAuthenticated"];

                if (isAuthenticated && auth == "ADM")
                {
                    ...
                    {
                        txtOrgName.Text = userOrganization;
                    }
        ...
        private void LoadEvents()
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                string query = "SELECT Organization, EventDate, StartTime, EndTime, Venue FROM Events WHERE Status = 4 ORDER BY EventDate ASC"; /*AND Organization<> 'OSAS'*/
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    conn.Open();
        ...
        protected void btnProposalTracking_Click1(object sender, EventArgs e)
        {
            Response.Redirect("ProposalTracking.aspx");
        }
